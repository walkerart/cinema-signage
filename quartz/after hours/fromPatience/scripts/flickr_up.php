...
$kwSetID = "72157602790387873";  //frida kahlo group id
$desc = "";
$tags = 'walkerartcenter kathyhalbriech';
...
function genTitle(){
	$wordlist = file("/Volumes/Patience/_after_hours/scripts/wordlist_ah.txt");
	$count = count($wordlist)-1;  //count
	$w1 = $wordlist[rand(0,$count)];
	$w2 = $wordlist[rand(0,$count)];
	//$title = "A ".trim($w1)." ".trim($w2)." Students";  //join them up
	$title = trim($w1)." ".trim($w2)." Kathies";  //join them up
